...
export class RelayWebsocket {
  ...
  private callbacks: Map<string, (payload: VscodeMessagePayload) => void>;
  ...
  static getInstance() {
    if (!RelayWebsocket.instance) {
      RelayWebsocket.instance = new RelayWebsocket(process.env.WS_RELAY_URL || "ws://localhost:9093/ws");
    }
