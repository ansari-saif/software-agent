...
export class ArtifactProcessor {
    ...
    parse() {
       const latestActionStart = this.currentArtifact.split("\n").findIndex((line) => line.includes("<clyneAction type="));
       const latestActionEnd = this.currentArtifact.split("\n").findIndex((line) => line.includes("</clyneAction>")) ?? (this.currentArtifact.split("\n").length - 1);
       ...
       if (latestActionType === "\"shell\"") {
        ...
        if (shellCommand.includes("</clyneAction>")) {
            shellCommand = shellCommand.split("</clyneAction>")[0];
        ...
        if (fileContent.includes("</clyneAction>")) {
            fileContent = fileContent.split("</clyneAction>")[0];
            this.currentArtifact = this.currentArtifact.split(latestActionContent)[1];
